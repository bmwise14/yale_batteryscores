...
def qids(input, nonresp):
    ...
    try:
        ...
        sleep = input[sleep_keys].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        sleepvalue = sleep[(sleep[sleep_keys] >= 0) & (sleep[sleep_keys] <= 3)].max(axis=1, skipna=True)
        sleepvalue.fillna(value=99)

        # ---------------------------
        weight = input[weight_keys].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        weightvalue = weight[(weight[weight_keys] >= 0) & (weight[weight_keys] <= 3)].max(axis=1, skipna=True)
        weightvalue.fillna(value=99)

        # ---------------------------
        psychomotor = input[psychomotor_keys].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        ...
        mood = input[mood_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        moodscore = mood[(mood[mood_key] >= 0)].sum(axis=1, skipna=True)
        moodscore.fillna(value=99)
        # ---------------------------
        concentration = input[concentration_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        concscore = concentration[(concentration[concentration_key] >= 0)].sum(axis=1, skipna=True)
        concscore.fillna(value=99)

        # ---------------------------
        selfcrit = input[self_criticism_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        critscore = selfcrit[(selfcrit[self_criticism_key] >= 0)].sum(axis=1,skipna=True)
        critscore.fillna(value=99)

        # ---------------------------
        suicidal = input[suicidal_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        ...
        # ---------------------------
        interest = input[interest_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
        interestscore = interest[(interest[interest_key] >= 0)].sum(axis=1, skipna=True)
        interestscore.fillna(value=99)

        # ---------------------------
        energy = input[energy_key].apply(pd.to_numeric, args=('coerce',), axis=1).replace(to_replace=[1, 2, 3, 4, nonresp['QIDS']],
                                                                                       value=[0, 1, 2, 3, 99])
